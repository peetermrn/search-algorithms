...
def pure_mc(board: Board, n=200):
    ...
    for move in board.available_moves:
        for _ in range(n):
            win_counts[move] += simulate(board.copy(), move)  # simulate returns 0, 1 or 1.5

    return max(win_counts, key=win_counts.get)


def play_game(board: Board):
    """Main game function. Player inputs move and computer uses monte carlo."""
    current = "Player"
    while True:
        if current == "Player":
            for line in board.pos:
                print(line)
